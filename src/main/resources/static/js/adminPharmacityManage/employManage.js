...
function fetchUsersSearch(page, size) {
	...
	let url = `/api/auth/employ/all?page=${page}&size=${size}`;
	if (userName) {
...
function fetchUsers(page) {
	fetch(`/api/auth/employ/all?page=${page}&size=${size}`)
		.then(res => res.json())
		.then(data => {
			if (data.success) {
...
function delRole(id) {
    ...
    $('#commitRoleDel').off('click').on('click', function () {
		fetch(`/api/auth/employ/delete?id=${id}`)
		.then(res => res.json())
		.then(data => {
			if (data.success) {
