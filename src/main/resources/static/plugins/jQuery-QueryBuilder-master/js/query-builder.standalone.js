...
var QueryBuilder = function($el, options) {
    ...
    // INIT
    this.$el.addClass('query-builder form-inline form-inline-1');
...
QueryBuilder.define('bt-selectpicker', function(options) {
    ...
    this.on('afterCreateRuleFilters', function(e, rule) {
        rule.$el.find(Selectors.rule_filter).removeClass('form-control').select2({'dropdownParent':rule.$el.find(Selectors.rule_filter).closest(".box-builder-class")});
    });

    this.on('afterCreateRuleOperators', function(e, rule) {
        rule.$el.find(Selectors.rule_operator).removeClass('form-control').select2({'dropdownParent':rule.$el.find(Selectors.rule_operator).closest(".box-builder-class")});
    });

    // update selectpicker on change
    this.on('afterUpdateRuleFilter', function(e, rule) {
        rule.$el.find(Selectors.rule_filter).select2({'dropdownParent': rule.$el.find(Selectors.rule_filter).closest(".box-builder-class")});
    });

    this.on('afterUpdateRuleOperator', function(e, rule) {
        rule.$el.find(Selectors.rule_operator).select2({'dropdownParent': rule.$el.find(Selectors.rule_operator).closest(".box-builder-class")});
    });
    ...
    setFilters: function(deleteOrphans, filters) {
        ...
        if (this.settings.plugins) {
            ...
            if (this.settings.plugins['bt-selectpicker']) {
                this.$el.find(QueryBuilder.selectors.rule_filter).select2({'dropdownParent':this.$el.find(QueryBuilder.selectors.rule_filter).closest(".box-builder-class")});
            }
        }
 ...
 * @param {object} [options]
 * @param {string} [options.icon_checked='fa fa-check-square-o']
 * @param {string} [options.icon_unchecked='fa fa-square-o']
 */
...
QueryBuilder.extend(/** @lends module:plugins.UniqueFilter.prototype */ {
    ...
    applyDisabledFilters: function(e) {
        ...
        if (self.settings.plugins && self.settings.plugins['bt-selectpicker']) {
            self.$el.find(QueryBuilder.selectors.rule_filter).select2({'dropdownParent': self.$el.find(QueryBuilder.selectors.rule_filter).closest(".box-builder-class")});
        }
    }
});
